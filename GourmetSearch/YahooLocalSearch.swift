...
//MARK: - APIExecuteClass
public class YahooLocalSearch {
    //Yahoo!ローカルサーチAPIのアプリケーションID
    let apiId = "dj0zaiZpPUdmS0ZUc1N6RUpPSCZzPWNvbnN1bWVyc2VjcmV0Jng9NTg-"
    //APIのベースURL
    ...
    var condition: QueryCondition = QueryCondition() {
        //プロパティオブザーバ： 新しい値がセットされた後に読込済みの店舗を捨てる
        didSet {
            shops = []
            total = 0
        }
    ...
    public func loadData(reset: Bool = false) {
        ...
        //APIリクエスト実行
        let request = Alamofire.request(apiUrl, method: HTTPMethod.get, parameters: params).response {
