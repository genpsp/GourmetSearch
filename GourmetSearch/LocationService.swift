...
//  Copyright © 2017年 梅垣玄. All rights reserved.
//
...
public class LocationService: NSObject, CLLocationManagerDelegate {
    ...
    public var locationServiceDisabledAlert: UIAlertController {
        get {
            let alert = UIAlertController(title: "位置情報が取得できません", message: "設定からプライバシー　→　位置情報画面を開いてGourmetSearchの位置情報の許可を「このAppの使用中のみ許可」に設定してください。", preferredStyle: .alert)
            alert.addAction(
    ...
    public var locationServiceRestrictedAlert: UIAlertController {
        get {
            let alert = UIAlertController(title: "位置情報が取得できません", message: "設定から一般　→　機能制限画面を開いてGourmetSearchが位置情報を使用できる設定にしてください。", preferredStyle: .alert)
            alert.addAction(
                UIAlertAction(title: "閉じる", style: .cancel, handler: nil)
            )
            return alert
